...
`Utils.clampBatchSize(requested, max)` keeps a refill batch between 1 and the cache capacity: a request of zero or less becomes 1, and anything above `max` becomes `max`.
